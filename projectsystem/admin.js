...
<body>
  ...
  // Auto-sync: ensure all registered employees (in credentials) exist in employees list
  function autoSyncEmployeesFromCredentials(){
    try{
      const creds = JSON.parse(localStorage.getItem('payroll_credentials') || '{}');
      let emps = JSON.parse(localStorage.getItem('payroll_employees') || '[]');
      const existingIds = emps.map(e => String(e.id));
      let added = 0;
      for(const empId in creds){
        if(existingIds.indexOf(String(empId)) === -1){
          const cred = creds[empId];
          emps.push({
            id: empId,
      ...
      if(added > 0){
        localStorage.setItem('payroll_employees', JSON.stringify(emps));
        console.log('Auto-synced ' + added + ' missing employee(s) from credentials');
      }
    }catch(e){ console.warn('Auto-sync failed:', e); }
  }

  // load and render on startup
  autoSyncEmployeesFromCredentials(); // Auto-sync before loading
  loadEmployeesFromStorage();
  ...
  // Also reload data when the admin tab gains focus (fallback for some browsers)
  window.addEventListener('focus', () => {
    autoSyncEmployeesFromCredentials();
    loadEmployeesFromStorage();
    loadAttendanceFromStorage();
    renderFiltered();
    renderAttendance();
  ...
  // Polling fallback: check every 3 seconds for changes to payroll_employees or credentials
  let _lastEmployeesJSON = localStorage.getItem('payroll_employees');
  let _lastCredentialsJSON = localStorage.getItem('payroll_credentials');
  setInterval(() => {
    try{
      const curCreds = localStorage.getItem('payroll_credentials');
      const curEmps = localStorage.getItem('payroll_employees');

      // If credentials changed, run auto-sync to add any new employees
      if(curCreds !== _lastCredentialsJSON){
        _lastCredentialsJSON = curCreds;
        autoSyncEmployeesFromCredentials();
      }

      // If employees changed (including from auto-sync), reload and re-render
      const newEmps = localStorage.getItem('payroll_employees');
      if(newEmps !== _lastEmployeesJSON){
        _lastEmployeesJSON = newEmps;
        loadEmployeesFromStorage();
        renderFiltered();
        updateHomeStats();
        try{ renderTimeClock(); }catch(e){}
